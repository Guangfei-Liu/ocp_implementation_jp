...
. Labels used to select and manage groups of pods in a single operation.
. Base name, `generateName`, is used in generating a unique pod name.
...
. _security context_ for containers that specifies whether they can run as
 privileged containers.
. External storage volumes should be mounted within the container.
...
For example, if you tag _pods_ with labels, _services_ can use label selectors
 to identify the pods which they represent.
...
The second example here shows how labeling a group of components with
...
* For list of build commands, see Developer's Guide: links:https://docs.openshift.com/enterprise/latest/architecture/core_concepts/builds_and_image_streams.html[https://docs.openshift.com/enterprise/latest/architecture/core_concepts/builds_and_image_streams.html]
