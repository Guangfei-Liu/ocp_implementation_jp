...
* Configure the DNS on the `oselab` server for your OpenShift Container Platform environment.
...
| `oselab.example.com`| Administration host; acts as the DNS server and NFS server and host from which you install the environment.
...
* In the lab exercises in this section, use the `oselab` host as your DNS and NFS server. Run remote commands on the OpenShift environment on the provisioning and staging host.
...
- Execute the commands on the rest of the nodes or hosts with a `for` loop or ansible to save time and effort.
- In some cases, in the interest of saving time, run commands directly on the nodes or hosts instead of using the `for` loop or ansible.
...
The OpenShift Container Platform installer configures hosts with SSH. In this section you create and install an SSH key pair on the `oselab` host and add the public key to the `authorized_hosts` file on the OpenShift hosts.
...
----
+
NOTE: In a different environment, you can adopt a nonroot user with `sudo` capabilities. For example, in Amazon Web Services (AWS), you adopt the `ec2-user` user.
...
* You will run many commands from both the `oselab` and `master1` hosts.
...
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-server-rpms
enabled=1
gpgcheck=0

[rhel-7-server-rh-common-rpms]
name=Red Hat Enterprise Linux 7 Common
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-server-rh-common-rpms
enabled=1
gpgcheck=0

[rhel-7-server-extras-rpms]
name=Red Hat Enterprise Linux 7 Extras
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-server-extras-rpms
enabled=1
gpgcheck=0

[rhel-7-server-optional-rpms]
name=Red Hat Enterprise Linux 7 Optional
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-server-optional-rpms
enabled=1
gpgcheck=0

[rhel-7-server-ose-3.5-rpms]
name=Red Hat Enterprise Linux 7 OSE 3.5
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-server-ose-3.5-rpms
enabled=1
gpgcheck=0

[rhel-7-fast-datapath-rpms]
name=Red Hat Enterprise Linux 7 Fast Datapath
baseurl=https://admin.shared.example.opentlc.com/repos/ocp/3.5//rhel-7-fast-datapath-rpms
enabled=1
gpgcheck=0

[rhel-7-server-ose-3.5-rpms]
name=Red Hat Enterprise Linux 7 OSE 3.5
baseurl=http://www.opentlc.com/repos/ose/3.5/rhel-7-server-ose-3.5-rpms
enabled=1
gpgcheck=0
...
Repository rhel-7-server-ose-3.5-rpms is listed more than once in the configuration
Repository rhel-7-fast-datapath-rpms is listed more than once in the configuration
repo id                                                              repo name                                                                        status
rhel-7-fast-datapath-rpms                                            Red Hat Enterprise Linux 7 Fast Datapath                                            15
rhel-7-server-extras-rpms                                            Red Hat Enterprise Linux 7 Extras                                                   76
rhel-7-server-optional-rpms                                          Red Hat Enterprise Linux 7 Optional                                              4,647
rhel-7-server-ose-3.5-rpms                                           Red Hat Enterprise Linux 7 OSE 3.5                                                 415
rhel-7-server-rh-common-rpms                                         Red Hat Enterprise Linux 7 Common                                                   82
rhel-7-server-rpms                                                   Red Hat Enterprise Linux 7                                                       4,876
repolist: 10,111
----

. 
. Install the following tools and utilities on the `oselab` host:
+
----
[root@oselab ~]# yum -y install wget git net-tools bind-utils iptables-services bridge-utils bash-completion
----
...
. Create an inventory file with groups used by Ansible:
+
----
...
Check that Ansible can reach all of the hosts:
...
-----
[root@oselab ~]# ansible -m ping all
master1 | SUCCESS => {
    "changed": false, 
    "ping": "pong"
}
...
In /etc/ansible/ansible.cfg find [privilege_escalation] section and uncomment 'become=True' line. After this change Ansible will try to run as root on all the nodes.
...
+
-----
ansible nodes -m copy -a 'src=/etc/yum.repos.d/open.repo dest=/etc/yum.repos.d/open.repo'

-----


=== Configure Wildcard DNS Entry on `oselab`
...
. On the administration host, `oselab`, do the following:
...
+
----
[root@oselab ~]# mkdir -v /var/named/zones
...
+
----
[root@oselab bin]# iptables -I INPUT -p tcp --dport 53 -s 0.0.0.0/0 -j ACCEPT ; \
iptables -I INPUT -p udp --dport 53 -s 0.0.0.0/0 -j ACCEPT ; \
...
. On the `oselab` host, run the following `for` loop to ensure that `NetworkManager` is installed on the master and all nodes:
+
[root@oselab ~]# ansible nodes -m yum -a 'name=NetworkManager'`.
...
[TIP]
=====
You can also use this Ansible command: `oselab`:
...
----
TIP: You can use this Ansible command to start and enable a service:
 `ansible -m service -a 'state=running enabled=true name=docker' nodes`

. On the `oselab` host, pull down the Docker images to `node1` and `node2` in the primary region:
+
----
...
In this exercise you run the Ansible playbook from the `oselab` host, which, in a real-world scenario, could be a laptop or a staging or provisioning server. No packages are deployed directly from `oselab` to the OpenShift nodes or master.
...
+
----
[root@master1 ~]# journalctl -f -u atomic-openshift-master -u atomic-openshift-node
----
...
+
----
--> Creating router trainingrouter ...
    secret "trainingrouter-certs" created
    warning: serviceaccounts "router" already exists
    clusterrolebinding "router-trainingrouter-role" created
...
+
----
[root@master1 ~]# mkdir -pv /root/pvs/{1..25}
...
[root@master1 ~]# mkdir -pv /root/pvs/{25..50}
