...
>>>>>>> 2044c898445444b81efd9bbb6b24a65b1019cc8c
...
=======
. Install the following tools and utilities on the `master1` host:
+
----
[root@oselab ~]# ssh master1.example.com "yum -y install wget git net-tools bind-utils iptables-services bridge-utils python-virtualenv gcc"
>>>>>>> 2044c898445444b81efd9bbb6b24a65b1019cc8c
----
...
+
----
[root@master1 ~]# yum -y install docker
----
...
+
----
[root@master1 ~]# sed -i "s/OPTIONS.*/OPTIONS='--selinux-enabled --insecure-registry 172.30.0.0\/16'/" /etc/sysconfig/docker
----
...
[root@master1 ~]# rm -rf /var/lib/docker/*
----
...
+
----
[root@oselab ~]# atomic-openshift-installer install
----
...
Enter hostname or IP address: []: master1.example.com
Will this host be an OpenShift Master? [y/N]: y
Do you want to add additional hosts? [y/N]: y
Enter hostname or IP address: []: infranode1.example.com
Do you want to add additional hosts? [y/N]: y
Enter hostname or IP address: []: node1.example.com
Do you want to add additional hosts? [y/N]: y
Enter hostname or IP address: []: node2.example.com
...
.. Paste or type the name of your master host `master1.example.com`.
.. Type `y` to confirm that this host is an OpenShift Master.
.. Type `y` to add more hosts.
.. Paste or type the name of your `infra` host `infranode1.example.com`.
.. Type `y` to add more hosts.
.. Paste or type the name of your `node1` host `node1.example.com`.
.. Type `y` to add more hosts.
.. Paste or type the name of your `node2` host `node2.example.com`
...
master1-GUID.oslab.opentlc.com,192.168.0.101,192.168.0.101,master1-GUID.oslab.opentlc.com,master1-GUID.oslab.opentlc.com
infranode1-GUID.oslab.opentlc.com,192.168.0.251,192.168.0.251,infranode1-GUID.oslab.opentlc.com,infranode1-GUID.oslab.opentlc.com
node1-GUID.oslab.opentlc.com,192.168.0.201,192.168.0.201,node1-GUID.oslab.opentlc.com,node1-GUID.oslab.opentlc.com
node2-GUID.oslab.opentlc.com,192.168.0.202,192.168.0.202,node2-GUID.oslab.opentlc.com,node2-GUID.oslab.opentlc.com
...
PLAY RECAP ********************************************************************
infranode1.example.com : ok=58   changed=22   unreachable=0    failed=0
localhost                  : ok=11   changed=0    unreachable=0    failed=0
master1.example.com : ok=206  changed=58   unreachable=0    failed=0
node1.example.com : ok=58   changed=22   unreachable=0    failed=0
node2.example.com : ok=58   changed=22   unreachable=0    failed=0
...
+
----
[root@master1 ~]# cat  /root/.config/openshift/installer.cfg.yml
...
. After the hosts finish rebooting, connect to the `master1` host:
+
----
[root@oselab ~]# ssh master1.example.com
----
...
=======
[root@master1 ~]# oadm registry --create --credentials=/etc/origin/master/openshift-registry.kubeconfig
>>>>>>> 2044c898445444b81efd9bbb6b24a65b1019cc8c
----
+
