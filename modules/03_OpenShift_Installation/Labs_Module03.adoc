...
. Add the OpenShift Container Platform repository mirror to the `oselab` host:
+
----
[root@oselab ~]# cat << EOF >> /etc/yum.repos.d/open.repo
...
----
TIP: You can also use this Ansible command: `ansible nodes -a "yum -y install NetworkManager"`.
...
. Install `bash-completion` on both the `oselab` and `master` hosts:
+
----
...
TIP: You can also use this Ansible command: `ansible nodes -a "yum -y install docker"`.
