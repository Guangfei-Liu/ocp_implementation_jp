...
[alice@master00-GUID ~]$ oc new-project wiring --display-name="Exploring Parameters" \
    --description='An exploration of wiring using parameters'

----

. Log into the web console as `alice`. Can you see `andrew`'s projects and content?
...
[alice@master00-GUID ~]$ oc process -f frontend-template.json > frontend-config.json
...
[alice@master00-GUID ~]$ oc create -f frontend-config.json
...
[alice@master00-GUID ~]$ oc process -f db-template.json \
...
[alice@master00-GUID ~]$ curl `oc get services | grep database | awk '{print $4}'`:5434
...
[alice@master00-GUID ~]$ oc get replicationcontroller
...
[alice@master00-GUID ~]$ oc describe rc ruby-hello-world-1
...
[alice@master00-GUID ~]$ oc delete pod `oc get pod | grep -e "hello-world-[0-9]" | grep -v build | awk '{print $1}'`
...
[alice@master00-GUID ~]$ oc get pod | grep world
...
[alice@master00-GUID ~]$ cd ~/training/beta4; oc create -f persistent-volume-claim.json
...
[alice@master00-GUID ~]$ oc get pvc
...
[alice@master00-GUID ~]$ oc edit dc/database
...
[alice@master00-GUID ~]$ oc delete pod `oc get pod | grep front | awk {'print $1'}`
...
[alice@master00-GUID ~]$ oc delete pod/`oc get pod | grep -e "database-[0-9]" | awk {'print $1'}`
