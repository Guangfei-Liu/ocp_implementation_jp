...
NOTE: "oselab" host is not an OpenShift Cluster memeber, and is not part of the
OpenShift environment, it mimics our client's infrastructure or our
 laptop/desktop connected to the client's LAN.

=== Important information and clearafications
...
NOTE: In our environemnt the OpenShift Default Router will be deployed on the
 "infranode00" host.
...
. Here is an example of a succesful connection:
+
----
...
[root@oselab-GUID ~]# for node in   master00-$guid.oslab.opentlc.com \
                                    ...
                                    echo removing NetworkManager on $node ; \
                                      ssh -o StrictHostKeyChecking=no $node "yum -y  remove NetworkManager*"
                                   done
----
...
/etc/sysconfig/docker-storage-setup: No such file or directory
...
metadata:
  annotations:
    openshift.io/node-selector: region=infra <1>
