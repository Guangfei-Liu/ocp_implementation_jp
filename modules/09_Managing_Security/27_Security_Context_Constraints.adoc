...
The restricted SCC ensures pods cannot run as privileged or host directory
 volumes, and requires that a pod runs as a user in a preallocated range of UIDs
  and with a pre-allocated MCS label.
