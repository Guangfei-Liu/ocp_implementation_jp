...
registry-claim Bound registry-storage 15Gi RWX 44s
----
...
. Run `oc volume` to modify the deployment configurations. Add the `registry-storage` volume to the registry's deployment configuration, which redeploys the registry:
+
----
