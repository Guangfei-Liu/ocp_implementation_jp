...
* If you have not set the `$GUID` and `$guid` environment variables, run the following commands:
+
----
...
=== Configure Repositories on Master Host
...
----
+
NOTE: `Broken Pipeline` messages in the output are normal and not an indication of errors.

. Enable, start, and obtain the status of the Docker service on the master host:
+
----
...
IMPORTANT: See the _<<Known Issues>>_ section if you have problems with Docker's storage setup.
...
.. Type `n` to confirm that this host is not an OpenShift Master.
.. Type `y` to add more hosts.
...
You now have a running OpenShift Enterprise environment across three hosts with one master and three nodes, divided into two regions: infra and primary. From here, you deploy applications and other resources into OpenShift Enterprise.
...
.. Connect to `https://master00-GUID.oslab.opentlc.com`.
...
+
[NOTE]
Once the Ansible-based installer automatically performs this task, we will delete this step from the lab.
