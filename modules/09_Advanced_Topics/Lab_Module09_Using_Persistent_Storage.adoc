...
[root@master00-GUID ~]# ]# for node in node00-$guid.oslab.opentlc.com node01-$guid.oslab.opentlc.com; do yum -y install nfs-utils ; done

----

. On the **oselab** admin host we will createi a directory we will export via NFS:
+
----
...
[root@oselab-GUID ~]# setsebool -P virt_use_nfs=true
...
[root@oselab-GUID ~]# wget https://raw.githubusercontent.com/openshift/training/master/beta4/persistent-volume.json
[root@oselab-GUID ~]# sed -i "s/ose3-master.example.com/oselab-$GUID.oslab.opentlc.com/" persistent-volume.json
[root@oselab-GUID ~]# oc create -f persistent-volume.json
...
[root@oselab-GUID ~]# oc describe persistentvolumes/pv0001
...
[marina@oselab-GUID ~]$ wget https://raw.githubusercontent.com/openshift/training/master/beta4/persistent-volume-claim.json
[marina@oselab-GUID ~]$ oc create -f persistent-volume-claim.json
...
[root@oselab-GUID ~]# oc describe pv/pv0001
...
[marina@oselab-GUID ~]$ oc edit dc/database
...
[marina@oselab-GUID ~]$ oc delete pod `oc get pod | grep front | awk {'print $1'}`
...
[marina@oselab-GUID ~]$ oc delete pod/`oc get pod | grep -e "database-[0-9]" | awk {'print $1'}`
