...
The example shown here uses the concept of _regions_ and _zones_.
...
* To enable writing in SELinux, installer performs on each node:
+
----
...
To configure SELinux to let the nodes use NFS in the way required for OpenShift
 Container Platform `Persistent Volumes`, the installer uses the `setsebool` command on each node, as
...
https://github.com/openshift/openshift-ansible/tree/release-1.5/roles/kube_nfs_volumes
