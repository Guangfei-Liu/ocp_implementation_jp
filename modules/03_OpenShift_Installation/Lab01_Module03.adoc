...
. Add the `Extra Packages for Enterprise Linux` or `EPEL` repository to the master host and disable it:
+
----
...
+
[NOTE]
At this time, the `ansible` package is only available from `EPEL`.
...
In this lab we will use Andible to install OpenShift on the master host.  This will configure the master host with the master and node roles.
