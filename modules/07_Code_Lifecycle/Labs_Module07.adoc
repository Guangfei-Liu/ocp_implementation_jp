...
If you look at the `frontend-config.json` file created in an earlier lab,
...
http://ruby-hello-world.wiring.cloudapps-GUID.oslab.opentlc.com/
...
. In the Github web GUI change to the `beta4` branch (top left).
+
[NOTE]
If you are unfamiliar with github, changing the branch could be tricky, make sure you do not skip this step.
...
. On the master host command line find the script called `custom-assemble.sh` in the `training/beta4` folder and paste the contents of `custom-assemble.sh` into the text area.
...
[alice@master00-GUID ~]$ osc get pod | grep -E "[0-9]-build" |\
   awk {'print $1'} | xargs -r osc delete pod
...
[alice@master00-GUID ~]$ osc get dc database -o yaml | grep -A1 MYSQL
...
[alice@master00-GUID ~]$ mysql -u[username] \
      -p[password] \
      -h[db_IP] \
      -P[db_PORT] \
...
[alice@master00-GUID ~]$ sed -i "s/cloudapps.example.com/cloudapps-$GUID.oslab.opentlc.com/" ~/training/beta4/wordpress-addition.json
...
[alice@master00-GUID ~]$ sed -i "s/cloudapps.example.com/cloudapps-$GUID.oslab.opentlc.com/" ~/training/beta4/eap6-basic-sti.json
